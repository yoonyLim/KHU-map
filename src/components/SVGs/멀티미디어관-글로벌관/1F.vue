...
<script setup>
    ...
    if (query != "") {
        selectedClass.value = query.value;
    }
...
<template>
<div v-if="floorInfo" class="w-full h-full flex flex-col">
...
<div v-if="selectedClass != ''">
    <div v-for="room in floorInfo.rooms" :key="room.id">
        <div v-if="(typeof room != 'object') && room == selectedClass">
            <span>해당 교실의 강의 정보는 없습니다!</span>
        </div>
        <div v-if="(typeof room == 'object') && room.roomNumber == selectedClass">
            <div class="flex flex-col">
                <span class="font-bold text-xl">해당 강의실 모든 강좌 정보</span>
                <div v-for="lecture in room.classes" :key="lecture.id">
                    <div class="my-4 p-4 border flex flex-col">
                        <div class="flex">
                            <span>{{ lecture.lectureName }}</span>
                            <span v-if="lecture.alias" class="ml-2">({{ lecture.alias }})</span>
                            <span class="ml-2">/ {{ lecture.Professor }}</span>
                        </div>
                        <div class="flex">
                            <span>날짜</span>
                        </div>
